...
//Now the HttPClient we will work 
  ...
  postEmployee(data : any){
    return this.http.post<any>("http://localhost:7777/api/users", data)
  ...
  getEmployee(){
   ...
   updateEmployee(data : UserCred ,id: number){
    return this.http.put<UserCred>("http://localhost:7777/api/users/"+id,data)
    .pipe(map((res:UserCred)=>{
      return res;
    }))
  }
  ...
  deleteEmployee(id : number){
    return this.http.delete<UserCred>("http://localhost:7777/api/users/"+id)
    .pipe(map((res:UserCred)=>{
      return res;
    }))
  }
